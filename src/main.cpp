...
void mainLoop(const SrcVec& srcs, bool doFMM, bool doIter = true) {
    ...
    Solver::printScattered(srcs, "out/ff/sph/px_k1.0z_r5.0_"+ieStr, (doFMM ? "ff_n" : "ffDir_n")+std::to_string(nsrcs)+"_par.txt", 200);
    // Solver::printScattered(srcs, "out/ff/plate/px_k1.0z_plate", std::string(doFMM ? "ff_g" : "ffDir_g")+config.lengStr+".txt", 100);
    // Solver::printScattered(srcs, "out/ff/almond/py_k146.6z_almond_"+ieStr, std::string(doFMM ? "ff" : "ffDir")+"_par.txt", 500);
